...
- `Controllers/DashboardController.cs` — `[Authorize(Policy="CmesUser")]`. Endpoints:
  - `GET /api/Dashboard/overview?date=YYYY-MM-DD` — selected day: KPIs, shifts(A/B/C), hourly(24, IST 06→05) — all 5 metrics. Each metric fetches that day's **scan timestamps** (helpers `NewEngineScans`/`PaintScans`/`TestCellScans`/`FesScans`, returning `List<DateTime>` — small, one row per engine), 5 run in PARALLEL (own contexts via factory); shift/hour/KPI counting done in C# (`ShiftOf`, `.Hour`). Returns **`plan`** (cycle-time target % per line, **O/N/T/P only** — NOT vs-previous-day): for each line `plan = round((working seconds from 06:00 IST to now − overlapping breaks) / cycleTime)`, `pct = (actual − plan)/plan` signed (▲ green ahead / ▼ red behind). **Cycle times** in `appsettings.json` `CycleTimes` (Old 150 / New 140 / Test 180 / Paint 180 sec). **Breaks** (8 fixed IST windows: 7:45-8, 10:30-11, 12:45-1, 4:45-5, 7-7:30, 8:45-9 + 1-1:30, 4-6 AM) excluded from plan time. Whole-day basis (effNow capped at day-end so past days = full day). Pure math, **no DB query** (so overview = just the 5 metric queries now, no prev-day batch). `hourly` is now **48 half-hour buckets** (`{time:"HH:mm", shift, ...}`) so shift split is exact at :30 (06:00/14:30/22:30). Live 30s refresh hits only this.
  - `GET /api/Dashboard/trends?year=YYYY` — **per-year** (default = current IST year). Each call bounded to `[Jan 1 year, Jan 1 year+1)` so it only ever touches ONE year (CREATEDON-index seek, never full history). backend `daily` = the **whole year** (Jan 1 → maxDay, continuous, 0-filled), `monthly` = that year's months. Per-day SQL `GROUP BY` (`PerDayNewEngines`/`PerDayPaint`/`PerDayTestCell`/`PerDayFes` → `DayCount` rows) run **in parallel** (own contexts), merged in C#. Cached 5 min under **per-year** key `trends_<year>`. **Frontend ties trends to the date picker (NO separate year dropdown):** trends is fetched for `effDate`'s year, the Daily chart shows only `effDate`'s **month** (filters the year's daily by month-abbrev), Monthly shows the year. So navigating the calendar changes both the day (overview) and the trend month/year.
...
- `pages/Dashboard.jsx` — the whole dashboard:
  - **5 KPI cards** (Old/New/Test/Paint/FES) with **cycle-time plan badge** (`ChangeBadge`: ▲ green ahead / ▼ red behind + "Plan N" target), fed by overview's `plan` — O/N/T/P only; FES has no plan (—).
  ...
  - **4 chart tabs:** Hourly / Shift / Daily / Monthly. Reusable `ProdChart` (**line or bar** toggle). **Hourly tab** = 24 hourly points (frontend sums the 48 half-hour buckets into hours, `hourlyByHour`). **Shift tab** = 3 graphs (A/B/C) using the raw 48 half-hour buckets filtered by each bucket's `shift` field (exact :30 boundaries). **Daily tab** = the **selected date's month** (`dailyMonth` filters the year's daily by month-abbrev). **Monthly tab** = the selected date's year. All trend tabs are driven by the date picker (no year dropdown).
...
- **Indexes are essential** — `database/indexes.sql` (HIST(WS,SER,CON), AMI(WS,CON), OUTBOUND(OVERALLSTATUS,CON) incl WIPJOBNO, SERIAL_NO(WORKORDERNO) incl SERIALNO,STATUS). **PLUS HIST needs `(WORKSTATION, CREATEDON)` composite** — without it the date-range queries (Old/New/Paint) seek only on WORKSTATION and scan that WS's whole history (date becomes a residual filter); the composite makes it a true WS+date seek (verified: Paint 294→6 logical reads). A plain `(CREATEDON)`-only index is NOT used by the optimizer when WORKSTATION is also filtered. Sir confirmed the real DB has the needed indexes (incl. WS+CREATEDON) → don't recreate on prod.
