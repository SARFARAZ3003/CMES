...
import api from '../api/client'   // axios (baseURL :5000/api, withCredentials) - Windows auth ke saath
...
async function fetchLocations() {
  // axios api client (baseURL :5000/api) -> GET /wip/locations. withCredentials se Windows auth flow.
  const res = await api.get('/wip/locations')
  const data = res.data
...
async function fetchDetails(location, page = 1, pageSize = 500) {
  ...
  const base = slug ? `/wip/details/${slug}` : '/wip/details'
...
export default function WipReport({ user, onLogout, page, onNavigate }) {
